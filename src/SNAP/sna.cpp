...
void SNA::compute_yi(double* beta)
{
  ...
  for(int j = 0; j <= twojmax; j++) {
    for(int mb = 0; 2*mb <= j; mb++)
      for(int ma = 0; ma <= j; ma++) {
        yarray_r[ielem][j][ma][mb] = 0.0;
        yarray_i[ielem][j][ma][mb] = 0.0;
      } // end loop over ma, mb
  } // end loop over j
  } // end loop over ielem
  ...
  for(int jjb = 0; jjb < idxj_max; jjb++) {
    const int j1 = idxj[jjb].j1;
    ...
    jjjzarray_r = zarray_r[idouble][j1][j2][j3];
    jjjzarray_i = zarray_i[idouble][j1][j2][j3];
    for(int mb = 0; 2*mb <= j; mb++)
      for(int ma = 0; ma <= j; ma++) {
        yarray_r[ielem][j][ma][mb] += betaj*jjjzarray_r[ma][mb];
        yarray_i[ielem][j][ma][mb] += betaj*jjjzarray_i[ma][mb];
      } // end loop over ma, mb

    j = j1;
    ielem = elem1;
    idouble = elem3*nelements+elem2;
    jjjzarray_r = zarray_r[idouble][j3][j2][j1];
    jjjzarray_i = zarray_i[idouble][j3][j2][j1];
    for(int mb = 0; 2*mb <= j; mb++)
      for(int ma = 0; ma <= j; ma++) {
        yarray_r[ielem][j][ma][mb] += betaj*jjjzarray_r[ma][mb];
        yarray_i[ielem][j][ma][mb] += betaj*jjjzarray_i[ma][mb];
    ...
    j = j2;
    ielem = elem2;
    idouble = elem3*nelements+elem1;
    jjjzarray_r = zarray_r[idouble][j3][j1][j2];
    jjjzarray_i = zarray_i[idouble][j3][j1][j2];
    for(int mb = 0; 2*mb <= j; mb++)
      for(int ma = 0; ma <= j; ma++) {
        yarray_r[ielem][j][ma][mb] += betaj*jjjzarray_r[ma][mb];
        yarray_i[ielem][j][ma][mb] += betaj*jjjzarray_i[ma][mb];
  ...
  for(int j = 0; j <= twojmax; j++) {
    ...
    for(int mb = 0; 2*mb < j; mb++)
      for(int ma = 0; ma <= j; ma++) {
        ...
        double jjjmambyarray_r = yarray_r[ielem][j][ma][mb];
        double jjjmambyarray_i = yarray_i[ielem][j][ma][mb];
        for(int k = 0; k < 3; k++)
          dedr[k] +=
...
void SNA::copy_bi2bvec()
{
    ...
    if(diagonalstyle == 0) {
      for(j2 = 0; j2 <= j1; j2++)
        for(j = abs(j1 - j2);
          ...
          bvec[ncount] = barray[itriple][j1][j2][j];
          ncount++;
        }
  ...
  itriple++;
  } // end loop over elem1,elem2,elem3
}
...
void SNA::compute_duidrj(double* rij, double wj, double rcut, int jelem)
{
  ...
  for(j1 = 0; j1 <= twojmax; j1++) {
    if(diagonalstyle == 0) {
      for(j2 = 0; j2 <= j1; j2++)
        for(j = abs(j1 - j2);
            j <= MIN(twojmax, j1 + j2); j += 2) {
          dbvec[ncount][0] = dbarray[itriple][j1][j2][j][0];
          dbvec[ncount][1] = dbarray[itriple][j1][j2][j][1];
          dbvec[ncount][2] = dbarray[itriple][j1][j2][j][2];
          ncount++;
        }
    } else if(diagonalstyle == 1) {
      j2 = j1;
...
void SNA::add_uarraytot(double r, double wj, double rcut, int ielem)
{
  double sfac;
...
void SNA::create_twojmax_arrays()
{
  ...
  memory->create(rootpqarray, jdim+1, jdim+1,
                 "sna:rootpqarray");
  memory->create(barray, ntriples, jdim, jdim, jdim,
                 "sna:barray");
  memory->create(dbarray, ntriples, jdim, jdim, jdim, 3,
                 "sna:dbarray");
