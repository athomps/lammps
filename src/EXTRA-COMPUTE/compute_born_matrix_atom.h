...
#ifdef COMPUTE_CLASS
// clang-format off
ComputeStyle(born/matrix/atom,ComputeBornMatrixAtom);
// clang-format on
#else

#ifndef LMP_COMPUTE_BORN_MATRIX_ATOM_H
#define LMP_COMPUTE_BORN_MATRIX__ATOM_H

#include "compute.h"

namespace LAMMPS_NS {

class ComputeBornMatrixAtom : public Compute {
 public:
  ComputeBornMatrixAtom(class LAMMPS *, int, char **);
  ~ComputeBornMatrixAtom() override;
  void init() override;
 ...
 private:
  ...
  int nmax;                          // allocated size
  double **born_matrix;              // per-atom Born matrix
};
}    // namespace LAMMPS_NS
